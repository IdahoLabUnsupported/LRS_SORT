...
namespace Sort.Business
{
    ...
    {
        public List<UserOrgObject> GetUserOrgs(string employeeId) => Config.Conn.Query<UserOrgObject>("SELECT * FROM dat_UserOrg EmployeeId = @EmployeeId ORDER BY Org", new { EmployeeId = employeeId }).ToList();
